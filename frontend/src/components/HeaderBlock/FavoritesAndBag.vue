  ...
  <slot name="right">
    <div class="RightIcons">
      <router-link to="/favorites" class="icon-heart text-[#838383] text-[7px] hover:text-red-700 lg:text-[22px]">
        <span class="Circle">{{ headphones.favoritesProducts.length + WirelessHeadphones.favoritesProducts.length }}</span>
      </router-link>
      <router-link to="/bag" class="icon-basket text-[#838383] text-[6px] hover:text-black lg:text-[22px]">
        <span class="Circle">{{ headphones.bagProducts.length + WirelessHeadphones.bagProducts.length }}</span>
      </router-link>
    </div>
  </slot>
</template>
